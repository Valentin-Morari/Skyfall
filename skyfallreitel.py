...
class Enemy:
	...
	def behavior(self): #The big glossary of shit monstas can do to ya!
		...
		if turn<>self.surname and not(self.path_recheck):
		...
		if turn==self.surname:
			...
			if (len(self.path) == 0 and (wherex,wherey) not in fway(self.tilex,self.tiley)) or self.path_recheck:
			...
			if self.name=='golem':
				...
				if self.CD['3'] <= 0 and (wherex,wherey) not in fway(self.tilex,self.tiley):  #Pull target towards self ALSO a check whether it's necessary
					...
					for coord in fway(self.tilex,self.tiley):
						for tile in tile_list:
							if tile.x == coord[0] and tile.y == coord[1] and tile.type in walkable_tiles_list and (time_player.tilex,time_player.tiley) <> coord:
								self.tilex = coord[0]
								self.tiley = coord[1]
