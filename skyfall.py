...
water=pygame.image.load("waturd.png").convert()
...
foxf=pygame.image.load('fox.png').convert()
foxf.set_colorkey(black)
...
infected={0:blank,1:grass_img,2:obstacle_img,3:grasst,4:grassd,5:grasslt,6:grassrt,7:grassld,8:grassrd,9:grassl,10:grassr,11:nclu,12:ncld,13:ncru,14:ncrd,15:grasshole,16:grasslone,17:grass2_img,18:grass3_img}
...
the_map=eval(open_map.readline())
map_data=eval(open_map.readline())
open_map.close() 

blinking={}

mode = '' #change to tutorial
	...
	def __init__(self,name,x,y,direction,parent,extra):
		...
		for i in range(len(enemy_list)):
				if enemy_list[i].tilex==self.tilex and enemy_list[i].tiley==self.tiley:
						self.dead=True
		for npc in npc_list:
			if npc.tilex == self.tilex and npc.tiley == self.tiley:
				self.dead=True
...
class Enemy:
	...
	def reposition(self):
		...
		for y in range(len(map_data)):
			for x in range(len(map_data[y])):
				if map_data[y][x]==-2 or map_data[y][x]==-3:
					for tile in potential_tiles:
						if tile.x==x*gw and tile.y==y*gh:
							potential_tiles.remove(tile)
	...
	def behavior(self): #The big glossary of shit monstas can do to ya!
		...
		if turn<>self.surname and self.path_recheck==False:
			
			self.path_recheck=True
				...
				if self.CD['3'] <= 0 and (wherex,wherey) not in fway(self.tilex,self.tiley):  #Pull target towards self ALSO a check whether it's necessary
					...
					for coord in fway(self.tilex,self.tiley):
						for tile in tile_list:
							...
							if tile.x == coord[0] and tile.y == coord[1] and tile.type in walkable_tiles_list and (wherex,wherey) <> coord:
								self.tilex = coord[0]
								self.tiley = coord[1]
								
								#Check if it's even legal to go there
								allowed=True
								for i in range(len(enemy_list)):
									if self.tilex==enemy_list[i].tilex and self.tiley==enemy_list[i].tiley and self<>enemy_list[i]:
										allowed=False
										break
										
								for i in range(len(projectile_list)):
									if projectile_list[i].name in  unwalkable_projectiles:
										if self.tilex==projectile_list[i].tilex and self.tiley==projectile_list[i].tiley:
											allowed=False
											break
								if (time_player.tilex,time_player.tiley) == (self.tilex,self.tiley):
									allowed=False
...
def restart():
	...
	if len(cloud_list)<>howmanyclouds:
		for i in range(howmanyclouds):
			...
			x=random.randint(0,int(size[0]+size[0]/3))
...
def Pull(initiator,target,maxDistance):
	...
	if distance > 0:
		...
		for npc in npc_list:
			if (npc.tilex,npc.tiley) in coords:
				RED_FLAG = False
				
		if (time_player.tilex,time_player.tiley) in coords and (time_player.tilex,time_player.tiley) <> (target.tilex,target.tiley):
			RED_FLAG = False
...
def tint(source,how,how_much):
	...
	return source
def notblocked(xtarget,ytarget):
	
	  for y in range(len(map_data)):
			for x in range(len(map_data[0])):
				if (map_data[y][x]==-2 or map_data[y][x] == -3) and (y*gh==ytarget) and (x*gw==xtarget):
					return False
...
class NPC:
	def __init__(self,x,y,who):
		self.who=who
		self.direction='down'
		self.image=image(self.direction,self.who,self)
		
		self.x=x/gw*gw+gw/2-self.image.get_width()/2
		self.y=y/gh*gh-self.image.get_height()+gh/2
		self.current_line=0
		self.text=open('text/'+str(self.who)+'.txt','r')
	...
	def create_text(self):
		try:
			text=Text(self.x-camX,self.y-camY+floatY,"Gautami",15,self.text_list[self.current_line],False,white)
			self.textRect=text.get_rect()
			self.textRect.center = ((self.x+self.image.get_width()/2-camX,self.y-camY+floatY-size[1]/50))
		except:
			text=text=Text(self.x-camX,self.y-camY+floatY,"Gautami",15,self.text_list[-1],False,white)
			self.textRect=text.get_rect()
			self.textRect.center = ((self.x-camX+self.image.get_width()/2,self.y-size[1]/50-camY+floatY))
			
		text.create()
		self.text_object=text
		
	def text_blit(self):
		self.text_object.blit(self.textRect)
...
class Player:
	...
	def walk(self,flag):
		...
		self.direction=flag
		for i in tile_list:
				if (self.tilex+flagx)==i.x and (self.tiley+flagy)==i.y and i.type in walkable_tiles_list and self.walk_check(flagx,flagy):
					self.y+=flagy
					self.x+=flagx
	...
	def walk_check(self,x,y):
		...
		for i in range(len(npc_list)):
			if self.tilex+x == npc_list[i].tilex and self.tiley+y == npc_list[i].tiley:
				allowed=False	
				break
				...
				self.direction=flag
					...
					for i in range(len(enemy_list)):
						if enemy_list[i].tilex==a.tilex and enemy_list[i].tiley==a.tiley:
								triggered=True
					for projectile in projectile_list:
						if projectile.name == 'bomb' and projectile.tilex==a.tilex and projectile.tiley==a.tiley:
							triggered=True
...
time_player=Time_Player(playerx,playery)
...
for y in range(len(the_map)):
	for x in range(len(the_map[y])):
				...
				elif the_map[y][x]==3 or the_map[y][x]==4 or the_map[y][x]==9 or the_map[y][x]==10 or the_map[y][x]==16 or the_map[y][x]==15 or the_map[y][x]==17 or the_map[y][x]==18:
					tile=Tile('grass',x*gw,y*gh,the_map[y][x])
					tile_list.append(tile)
...
def blit_order():
	...
	sorted_blits=[]
	try:
		npc_list[0].calc_tile()
	except:
		pass
	for i in range(len(npc_list)):
		if npc_list[i].image.get_height()>gh:
			npc_list[i].calc_tile()
			blitlist[npc_list[i]]=[npc_list[i].tiley,npc_list[i].tilex]
	for i in range(len(enemy_list)):
	...
	if switch:
		...
		player.calc_tile(True)
		blitlist[player]=[player.tiley,player.tilex]
...
def finalfind(x,y,x2,y2,w,h):
	...
	for npc in npc_list:
		npc.calc_tile()
		if (npc.tilex,npc.tiley) in walkablelist:
			delete.append((npc.tilex,npc.tiley))
...
for y in range(len(map_data)):
	for x in range(len(map_data[y])):
		if map_data[y][x]==-3:
			foxy=NPC(x*gw,y*gh,'fox')
			npc_list.append(foxy)
...
if __name__ == '__main__':
	...
	while done == False:
		...
		#						 print tile_list[i].x,tile_list[i].y
			...
			if not(switch):
				camC=(player.x,player.y)
			elif turn=='Time_Player':
